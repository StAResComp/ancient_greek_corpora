...
// SQL statements
#define INSERT_TEXT "INSERT INTO texts (text_id, text_name, title, settlement, region, earliest, latest, person) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?);"
   ...
   if (NULL != form) 
     {
        ...
        if (0 == regexec(&greek, form, 0, NULL, 0)) 
          {
             // try to insert unique form
             if (SQLITE_OK != sqlite3_bind_text(insert_unique_stmt, 1, form, -1, SQLITE_TRANSIENT) ||
             ...
             if (SQLITE_OK != sqlite3_bind_text(select_unique_stmt, 1, form, -1, SQLITE_TRANSIENT) ||
...
int xmlfile(xmlTextReaderPtr reader, char *text_name) /*{{{*/
{
   const char *elname = NULL;
   char *value;
   int i, a = 0, rc = INGEST_ERROR, p, t = 1, j, text_id;

   if (SQLITE_OK != sqlite3_bind_text(insert_text_stmt, 1, 
   ...
   for (j = 2; j < argc; ++ j) 
     {
        ...
        if (INGEST_END != xmlfile(reader, text_name)) 
          {
